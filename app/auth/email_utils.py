...
def send_verification_email(to_email: str, token: str) -> None:
    ...
    if not SMTP_HOST or not SMTP_USER:
        print(f"\n{'='*60}\n[EMAIL VERIFICATION] {to_email}\nClick to verify: {verify_url}\n{'='*60}\n", flush=True)
        logger.info("Dev mode: verification link printed to console for %s", to_email)
        return
    msg = MIMEMultipart("alternative")
    msg["Subject"] = "Verify your Ask My Docs account"
    msg["From"] = FROM_EMAIL
    msg["To"] = to_email
    msg.attach(MIMEText(
        f"<html><body style='font-family:sans-serif;padding:32px'>"
        f"<h2>Welcome to Ask My Docs</h2>"
        f"<p>Click the button below to verify your email address:</p>"
        f"<a href='{verify_url}' style='display:inline-block;padding:12px 24px;background:#c6f24a;color:#0c1003;border-radius:8px;text-decoration:none;font-weight:600'>Verify Email</a>"
        f"<p style='margin-top:24px;color:#666'>If you did not register, ignore this email.</p>"
        f"</body></html>",
        "html",
    ))
    with smtplib.SMTP(SMTP_HOST, SMTP_PORT) as s:
